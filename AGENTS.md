...
- 最近提交已包含 `v0.1.0`、`v0.1.1`、`v0.1.2`、`v0.2.0`、`v0.2.1` 与 `v0.2.2`，当前工作区主要进入 `v0.2.3` 国际化与侧边栏刷新联动版本的发布收尾
- 仓库已完成 `v0.2.2` 发布，当前正在整理并发布 `v0.2.3`，重点覆盖插件中英文显示语言切换、切换语言后的缓存刷新联动与引用统计文案简化
- 当前候选栏样式、点击选择、回车插入与越界修正已基本稳定，本轮新增了插件显示语言设置，并打通了语言切换与侧边栏刷新之间的联动
...
- 设置页当前新增插件显示语言选择，仅提供 `English` 与 `简体中文` 两项；切换后会立即重建 i18n、刷新缓存并重绘侧边栏
...
- 在 Typora 中回归显示语言切换后的设置页、侧边栏标题、按钮文案与缓存刷新表现
...
- 若继续扩展侧边栏，优先围绕当前文档引用统计增加更多上下文，例如最近引用 key、缺失 key 或文件级分布
