...
- 当前已接入 bibliography MVP：可从正文里的严格 `[@key]` 与受控 citation 块中提取 key，生成、更新或删除文末受控参考文献块；citation 也支持基于受控块直接按最新 CSL 重新渲染
...
- `Render / Update Citations` 当前会同时处理严格合法的可见 `[@key]` / `[@a; @b]` 闭合块与已有受控 citation 块；带前缀说明、locator、未知 key 或逗号分隔的可见块会保持原样
...
- 统一引用源提取当前同时识别两类来源：正文里直接可见的严格 `[@key]` 与受控 citation 注释中保存的原始 `[@key]`；bibliography、当前文档引用统计与相关校验都应优先复用这套提取器
...
- 侧边栏按钮当前布局约定是：`Refresh Cache` 独占一行，`Render / Update Citations` 与 `恢复` 同行七三开，`Insert / Update Bibliography` 与 `删除` 同行七三开
...
- 在 Typora 真机里继续回归 `CSL File` 路径配置、侧边栏按钮、citation 渲染/更新/恢复与插件启动稳定性
