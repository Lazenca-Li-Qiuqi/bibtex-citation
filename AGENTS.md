...
- 当前工作区目录名已迁移为 `bibtex-citation`；Git 远程仓库名暂时仍为 `typora-plugin-zotero`
...
- 如需彻底完成品牌迁移，可在合适时机把仓库目录名和远程仓库名一并调整为 `bibtex-citation`
...
- 若候选项不出现，先检查设置页里的 BibTeX 文件路径是否正确，再确认这些文件存在且可读
